...
export default async function SponsorDetailsPage({
  ...
  const relatedSponsors = await prisma.sponsor.findMany({
    where: {
      name: {
        equals: sponsor.name,
        mode: "insensitive",
  ...
  const highestTierSponsor = relatedSponsors.reduce((current, item) => {
    const currentScore = TIER_PRIORITY[current.tier] ?? 0;
    const itemScore = TIER_PRIORITY[item.tier] ?? 0;
    return itemScore > currentScore ? item : current;
  }, sponsor);

  const sponsoredEvents = Array.from(
    new Map(
      relatedSponsors.map((item) => [item.event.id, item.event]),
    ).values(),
  );
  ...
  return (
    <div className="flex flex-col gap-6 w-full animate-in fade-in duration-500 pb-10">
      ...
      <div className="relative rounded-3xl border border-border/50 bg-card/40 overflow-hidden backdrop-blur-sm shadow-sm group">
        ...
        <div
          className={cn(
            "h-32 sm:h-48 w-full bg-gradient-to-br relative transition-all duration-700 ease-in-out",
            getTierGlow(highestTierSponsor.tier),
          )}
        >
          ...
          <div className="absolute top-4 right-4 z-20">
            <Badge
              variant="outline"
              className={cn(
                "uppercase backdrop-blur-md bg-background/50 text-[10px] font-bold tracking-widest px-3 py-1 border shadow-sm",
                getTierColor(highestTierSponsor.tier),
              )}
            >
              {getTierIcon(highestTierSponsor.tier)}
              <span className="ml-1.5">{highestTierSponsor.tier} Partner</span>
            </Badge>
          </div>
        </div>
        ...
        <div className="px-6 sm:px-10 pb-8 pt-0 relative z-10 flex flex-col sm:flex-row gap-6 sm:gap-8 items-start sm:items-end -mt-12 sm:-mt-16">
          <div className="h-24 w-24 sm:h-32 sm:w-32 rounded-3xl bg-background p-2.5 shrink-0 border border-border/50 shadow-xl transition-transform duration-500 group-hover:scale-[1.02]">
            <div
              className={cn(
                "h-full w-full rounded-2xl flex items-center justify-center bg-gradient-to-br",
                getTierGlow(highestTierSponsor.tier),
              )}
            >
              {sponsor.logo ? (
                ...
                <span
                  className={cn(
                    ...
                    highestTierSponsor.tier.toLowerCase() === "platinum"
                      ? "text-slate-300"
                      : highestTierSponsor.tier.toLowerCase() === "gold"
                        ? "text-amber-500"
                        : highestTierSponsor.tier.toLowerCase() === "silver"
                          ? "text-slate-400"
                          : "text-orange-600",
                  )}
                >
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto mt-4 sm:mt-0 mb-2">
            <Link href={`/sponsors/${sponsor.id}/edit`}>
              <Button
                variant="outline"
                className="w-full sm:w-auto rounded-xl bg-background/50 backdrop-blur-sm border-border"
              >
            ...
            <DropdownMenu>
              <DropdownMenuTrigger>
                <Button
                  variant="outline"
                  size="icon"
                  className="rounded-xl shrink-0 bg-background/50 backdrop-blur-sm border-border"
                >
              ...
              <div className="space-y-4">
                <div className="flex gap-3.5 text-sm group/item">
                  ...
                  <div className="flex flex-col justify-center">
                    <span className="text-muted-foreground text-[11px] uppercase tracking-wider font-semibold mb-0.5">
                      Primary Contact
                    </span>
                ...
                <div className="flex gap-3.5 text-sm group/item">
                  ...
                  <div className="flex flex-col justify-center">
                    ...
                    <span
                      className={cn(
                        "text-foreground/90 font-medium leading-tight capitalize",
                        highestTierSponsor.tier.toLowerCase() === "platinum"
                          ? "text-slate-300"
                          : highestTierSponsor.tier.toLowerCase() === "gold"
                            ? "text-amber-500"
                    ...
                    >
                      {highestTierSponsor.tier.toLowerCase()} Tier
                    </span>
                  </div>
                </div>
                ...
                <div className="flex gap-3.5 text-sm group/item">
                  <div className="h-8 w-8 rounded-full bg-muted/50 flex items-center justify-center shrink-0 group-hover/item:bg-primary/10 transition-colors border border-border/50">
                    <CalendarIcon className="h-4 w-4 text-muted-foreground group-hover/item:text-primary transition-colors" />
                  </div>
                  <div className="flex flex-col justify-center">
                    <span className="text-muted-foreground text-[11px] uppercase tracking-wider font-semibold mb-0.5">
                      Sponsored Events
          ...
          <div className="flex items-center justify-between border-b border-border/50 pb-4">
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                This sponsor is currently partnered with the following events.
              </p>
            </div>
            <Badge variant="outline" className="rounded-xl">
              {sponsoredEvents.length} Events
            </Badge>
          </div>
                ...
                <div className="h-32 bg-muted relative border-b border-border/50 shrink-0 overflow-hidden">
                  ...
                  {event.banner && (
                    <img
                      src={event.banner}
                      alt={event.title}
                      className="w-full h-full object-cover mix-blend-overlay opacity-50 z-0"
                    />
                  )}
